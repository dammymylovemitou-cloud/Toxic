 ...
 * - Pair Code implementation inspired by MINATO MD
 */
require('./settings')
...
setInterval(() => {
    ...
    if (used > 400) {
        console.log(`⚠️ RAM too high (${used.toFixed(2)}MB > 400MB), restarting bot...`)
...
async function startXeonBotInc() {
    try {
        ...
        const XeonBotInc = makeWASocket({
            ...
            syncFullHistory: false, // Changed to false to reduce memory usage
        ...
        XeonBotInc.ev.on('connection.update', async (update) => {
            ...
            if (connection == "open") {
                ...
                try {
                    ...
                    console.log("Couldn't send connection message:", e.message)
                }

                await delay(1999)
                console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ ${global.botname || 'MINATO MD'} ]`)}\n\n`))
        ...
        // Message handling - SINGLE handler for all messages
        XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
            try {
                ...
                // Skip if bot is not public and message is not from me
                if (!XeonBotInc.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
                
                // Skip certain message IDs
                if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return

                // Clear message retry cache to prevent memory bloat (optional)
                if (msgRetryCounterCache && Date.now() % 60000 < 1000) {
        ...
        if (pairingCode && !XeonBotInc.authState.creds.registered) {
            ...
            if (!!global.phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFormat: 8801719741293 (without + or spaces) : `)))
            }
            ...
            if (!pn('+' + phoneNumber).isValid()) {
                console.log(chalk.red('Invalid phone number. Please enter your full international number (e.g., 92323xxxxxxx for PK, 9232370xxx for UK, etc.) without + or spaces.'))
                process.exit(1)
            }
